...
pub struct CartController {
    service: CartService,
    cart_and_product_service: CartAndProductService,
}


impl CartController {
    pub fn new(service: CartService, cart_and_product_service: CartAndProductService) -> Self {
        Self { service, cart_and_product_service }
    }
    ...
    pub async fn get_cart(&self, user: Option<UserClaims>) -> Result<Json<CartProductsWithTotals>, (StatusCode, String)> {
        ...
        let cart_products = self.cart_and_product_service.get_cart(user)
            .await
            .map_err(|e| (StatusCode::BAD_REQUEST, e.to_string()))?;
