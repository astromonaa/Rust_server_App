...
fn build_controller(connection: &Arc<DatabaseConnection>) -> Arc<OrderController> {
    ...
    Arc::new(OrderController::new(order_service))
}
...
pub fn setup_router(connection: DatabaseConnection) -> anyhow::Result<Router> {
    ...
    let router = Router::new()
        ...
        .route("/:order_id/status", put(order_router_helper::update_order_status).route_layer(from_fn(get_user_middleware)))
